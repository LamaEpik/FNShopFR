...
class Athena:
    ...
    def main(self):
        print("Générateur de la boutique d'objet")
        print("Crée par : https://github.com/EthanC/Athena\n")
        print("Modifié par : https://github.com/PokeSantiTW/Spanish-Twitter-Item-Shop-Bot\n")
        print("Modifié par : https://github.com/LamaEpik/FNShopFR\n")

        initialized = Athena.LoadConfiguration(self)

        if initialized is True:
            if self.delay > 0:
                log.info(f"Retarder l'exécution de {self.delay}s...")
                sleep(self.delay)
            ...
            if itemShop is not None:
                ...
                log.info(f"Récupération de la boutique du {date}")
    ...
    def GenerateImage(self, date: str, itemShop: dict):
        ...
        canvas.text((20, 255), "Offres", (255, 255, 255), font=font)
        textWidth, _ = font.getsize("Daily")
        canvas.text(
            (shopImage.width - (textWidth + 130), 255),
            "Quotidien",
            (255, 255, 255),
            font=font,
        )
        ...
        try:
            shopImage.save("itemshop.jpg")
            log.info("Image de la boutique générée !")
    ...
    def Tweet(self, date: str):
        ...
        """
        Tweet the current `itemshop.jpg` local file to Twitter using the credentials provided
        ...
        try:
            ...
            log.critical(f"Erreur lors de l'authentification a Twitter, {e}")

            return

        body = f"Boutique Fortnite d'aujourd'hui !"
        ...
        try:
            with open("itemshop.jpg", "rb") as shopImage:
                twitterAPI.PostUpdate(body, media=shopImage)

            log.info("Tweet envoyé !")
        except Exception as e:
            log.critical(f"Erreur lors de l'envoie du tweet, {e}")
