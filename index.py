...
def get_content(session: Optional[requests.Session] = requests.Session()) -> dict:
    ...
    if res.status_code != 200:
        print(f'Impossible de récupérer les données de la boutique\n{res.text}', file=sys.stderr)
...
def generate_image(data: dict, colors: dict, session: Optional[requests.Session] = requests.Session()) -> Image.Image:
    print(f"Génération de la boutique d'objet avec {len(data['sections'])} catégories")
    start = time.time()
    ...
    for count, future in enumerate(futures, 1):
        try:
            ...
            print('Impossible de générer les catégories', file=sys.stderr)
            traceback.print_exc()
        else:
    ...
    print(f"Photo de la boutique généré en {end - start:.2f} secondes")
    return image
...
print('Récupération des données de la boutique')
...
print(f'Photo sauvegardé en {end - start:.2f} seconds')
